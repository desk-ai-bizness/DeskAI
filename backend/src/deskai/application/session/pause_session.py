"""Pause a real-time consultation session."""
...
class PauseSessionUseCase:
    """Validate state, transition session to PAUSED, and audit."""
    ...
    def execute(self, consultation_id: str, doctor_id: str) -> Session:
        ...
        SessionService.validate_pause(session.state)
        ...
        self.audit_repo.append(
            AuditEvent(
                event_id=new_uuid(),
                consultation_id=consultation_id,
                event_type=AuditAction.SESSION_PAUSED,
                actor_id=doctor_id,
                timestamp=now,
