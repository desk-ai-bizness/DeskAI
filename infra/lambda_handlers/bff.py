...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    _exact_routes: dict[tuple[str, str], Any] = {
        ("/v1/auth/session", "POST"): lambda: auth_handler.handle_login(event, container),
        ("/v1/auth/session", "DELETE"): lambda: auth_handler.handle_logout(event, container),
        ("/v1/auth/forgot-password", "POST"): lambda: auth_handler.handle_forgot_password(
            event, container
        ),
        (
            "/v1/auth/confirm-forgot-password",
            "POST",
        ): lambda: auth_handler.handle_confirm_forgot_password(event, container),
        ("/v1/me", "GET"): lambda: me_handler.handle_get_me(event, container),
        ("/v1/consultations", "POST"): lambda: consultation_handler.handle_create_consultation(
            event, container
        ),
        ("/v1/consultations", "GET"): lambda: consultation_handler.handle_list_consultations(
            event, container
        ),
        ("/v1/patients", "POST"): lambda: patient_handler.handle_create_patient(event, container),
        ("/v1/patients", "GET"): lambda: patient_handler.handle_list_patients(event, container),
        ("/v1/ui-config", "GET"): lambda: ui_config_handler.handle_get_ui_config(event, container),
    }
    ...
    _param_routes = [
        ...
        (
            re.compile(r"^/v1/consultations/(?P<id>[^/]+)/review$"),
            "GET",
            review_handler.handle_get_review,
        ),
        (
            re.compile(r"^/v1/consultations/(?P<id>[^/]+)/review$"),
            "PUT",
            review_handler.handle_update_review,
        ),
        (
            re.compile(r"^/v1/consultations/(?P<id>[^/]+)/finalize$"),
            "POST",
            finalize_handler.handle_finalize,
        ),
        (
