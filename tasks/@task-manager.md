...
### Current Phase
Authenticated app core workflow corrections planned — public website deferred. Task 023 scope locked with client-side ElevenLabs streaming, async finalization, pause/resume, chunk-key fix, and schema-only provisional updates. Task 025 added for auth token hardening.
...
- 2026-04-19 planning session:
  ...
  - Task 025 created for auth token delivery hardening (resolves OI-007) — scheduled after Task 013 and before Task 014 so the security hardening pass inherits the new token strategy.
...
| --- | --- | --- | --- | --- |
| 1 | `023-implement-realtime-consultation-data-feed.md` | Implement realtime consultation data feed | planned | Removes stub transcript behavior, adds async finalization (OI-015), pause/resume, schema-only provisional events, and fixes chunk-key collision |
...
| 3 | `024-redesign-consultation-workspace-with-anamnese-and-inline-review.md` | Redesign consultation workspace with Anamnese and inline review | planned | Makes the live consultation screen the core app workspace and emits the provisional events defined in Task 023 |
...
| 5 | `025-harden-auth-token-delivery.md` | Harden authentication token delivery | planned | Moves refresh token to HttpOnly cookie and drops sessionStorage before security hardening pass (OI-007) |
...
| 023 | `023-implement-realtime-consultation-data-feed.md` | Implement realtime consultation data feed | Full Stack | Critical | planned | `008-implement-realtime-consultation-session-transport.md`, `009-integrate-transcription-provider-and-normalization.md`, `010-build-ai-processing-pipeline-and-artifacts.md` | 0 | Client-side ElevenLabs Realtime streaming with short-lived token endpoint, async finalization via EventBridge (OI-015), pause/resume, schema-only `insight.provisional` + `autofill.candidate` events (emitter in Task 024), chunk-key collision fix, and stub transcript removal (OI-016) |
...
| Frontend Ready | Core MVP interface in place | in_progress | Authenticated app polish completed through Task 020. Tasks 021-024 plan core workflow corrections before Task 013 public website work. |
...
| OI-015 | Finalization runs synchronously inside `session.stop` handler | `FinalizeTranscriptUseCase` is called inline after session ends. Long transcriptions could exceed Lambda 30s timeout. Handler catches and logs exceptions to avoid breaking the stop response. | open | Resolve in `023-implement-realtime-consultation-data-feed.md`: `session.stop` emits a `consultation.session.stopped` EventBridge event; a separate finalization Lambda handles transcript persistence + AI pipeline trigger with retries and DLQ. |
| OI-016 | Stub `transcript.partial` still sent in `audio.chunk` handler | Real-time partial transcripts from ElevenLabs require persistent WebSocket or callback. Current handler still sends `[stub transcript]` placeholder. The actual transcription happens in batch when `fetch_final_transcript` is called. | open | Resolve in `023-implement-realtime-consultation-data-feed.md`: client-side direct streaming to ElevenLabs Scribe v2 Realtime using a short-lived backend-issued token; stub events removed everywhere. |
| OI-018 | In-memory audio chunk buffer `_SessionEntry.chunk_count` is not globally unique across concurrent Lambda containers | S3 audio chunk keys can collide when multiple warm Lambda containers handle `audio.chunk` messages for the same session, causing chunks to overwrite each other and `session.stop` to report `audio_bytes: 0`. | open | Resolve in `023-implement-realtime-consultation-data-feed.md`: either retire the backend `audio.chunk` path in favor of client-side streaming, or make the S3 chunk key globally unique using `chunk_index` or uuid, and reassemble in client-issued order. |
