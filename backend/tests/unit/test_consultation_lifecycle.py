...
class ValidateTransitionTest(unittest.TestCase):
    ...
    def test_draft_generated_to_under_physician_review(self) -> None:
        self.assertTrue(validate_transition(S.DRAFT_GENERATED, S.UNDER_PHYSICIAN_REVIEW))

    def test_under_physician_review_to_finalized(self) -> None:
        self.assertTrue(validate_transition(S.UNDER_PHYSICIAN_REVIEW, S.FINALIZED))

    def test_under_physician_review_to_itself_is_allowed(self) -> None:
        self.assertTrue(validate_transition(S.UNDER_PHYSICIAN_REVIEW, S.UNDER_PHYSICIAN_REVIEW))
...
class ForbiddenTransitionsTest(unittest.TestCase):
    ...
    def test_finalized_to_under_physician_review_is_forbidden(self) -> None:
        self.assertFalse(validate_transition(S.FINALIZED, S.UNDER_PHYSICIAN_REVIEW))
...
class TransitionConsultationTest(unittest.TestCase):
    """Test transition_consultation returning new frozen instances."""
    ...
    def test_original_consultation_unchanged_after_transition(self) -> None:
        c = _make_consultation(S.STARTED)
        transition_consultation(c, S.RECORDING)
        self.assertEqual(c.status, S.STARTED)
        self.assertEqual(c.updated_at, "")

    def test_transition_sets_updated_at(self) -> None:
        c = _make_consultation(S.STARTED)
        result = transition_consultation(c, S.RECORDING)
        self.assertNotEqual(result.updated_at, "")
    ...
    def test_transition_to_finalized_sets_finalized_fields(self) -> None:
        ...
        self.assertEqual(result.status, S.FINALIZED)
        self.assertEqual(result.finalized_at, "2026-04-01T12:00:00+00:00")
        self.assertEqual(result.finalized_by, "doc-1")
    ...
    def test_transition_to_processing_failed_sets_error_details(self) -> None:
        ...
        result = transition_consultation(c, S.PROCESSING_FAILED, error_details=error_info)
        self.assertEqual(result.status, S.PROCESSING_FAILED)
        self.assertEqual(result.error_details, error_info)

    def test_under_physician_review_to_itself_is_idempotent(self) -> None:
        c = _make_consultation(S.UNDER_PHYSICIAN_REVIEW)
        result = transition_consultation(c, S.UNDER_PHYSICIAN_REVIEW)
        self.assertEqual(result.status, S.UNDER_PHYSICIAN_REVIEW)
