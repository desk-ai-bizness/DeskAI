...
| OI-005 | Specialty list and validation approach not defined | Domain model cannot validate specialty field | resolved | MVP scoped to `general_practice` only per CLAUDE.md ("limited to general practice/generalist consultations"). Expand post-MVP when additional specialties are needed. |
