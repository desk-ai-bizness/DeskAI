...
| Frontend Ready | Core MVP interface in place | in_progress | Authenticated app workflow corrections advanced through Task 022. Task 024 now owns the unified consultation workspace before public website work. |
...
| OI-019 | Dev/prod frontend custom domains are configured in app/infra docs but are not yet reachable | The AWS `dev` and `prod` frontend URLs (`app.dev.deskai.com.br`, `dev.deskai.com.br`, `app.deskai.com.br`, `deskai.com.br`) may remain inaccessible until the domain is purchased and CloudFront/ACM/DNS records are fully configured. This blocks browser access to the deployed frontend through the intended URLs. | open | Handle during `013-build-public-website-and-entry-flows.md` or a dedicated infra follow-up: confirm domain ownership, provision ACM certificates in the correct region, attach custom domains to CloudFront, and create DNS records. |
