...
class CognitoAuthProvider(AuthProvider):
    ...
    def __init__(
        ...
        self._region = user_pool_id.split("_")[0] if "_" in user_pool_id else "us-east-1"
        self._issuer = f"https://cognito-idp.{self._region}.amazonaws.com/{user_pool_id}"
    ...
    def authenticate(self, email: str, password: str) -> Tokens:
        try:
            ...
            if error_code == "UserNotConfirmedException":
                raise AccountNotVerifiedError("Email has not been verified.") from exc
            if error_code == "UserNotFoundException":
                raise AuthenticationError("Invalid email or password.") from exc
            raise AuthenticationError(f"Authentication failed: {error_code}") from exc
    ...
    def sign_out(self, access_token: str) -> None:
        try:
            ...
            raise AuthenticationError("Sign-out failed.") from exc
    ...
    def forgot_password(self, email: str) -> None:
        try:
            ...
            if error_code == "InvalidParameterException":
                return
            raise AuthenticationError("Password reset failed.") from exc
    ...
    def confirm_forgot_password(
        ...
        try:
            ...
            if error_code in (
                ...
                raise AuthenticationError("Invalid or expired confirmation code.") from exc
            if error_code == "UserNotFoundException":
                raise AuthenticationError("Invalid or expired confirmation code.") from exc
            raise AuthenticationError("Password confirmation failed.") from exc
    ...
    def validate_ws_token(self, token: str) -> dict:
        ...
        # Try custom Cognito attributes first, fall back to DynamoDB lookup.
        doctor_id = claims.get("custom:doctor_id", "")
        clinic_id = claims.get("custom:clinic_id", "")

        if (not doctor_id or not clinic_id) and self._doctor_repo:
            profile = self._doctor_repo.find_by_identity_provider_id(sub)
            if profile is None:
                raise AuthenticationError("No doctor profile found for this user.")
            doctor_id = doctor_id or profile.doctor_id
            clinic_id = clinic_id or profile.clinic_id

        if not doctor_id or not clinic_id:
            raise AuthenticationError(
                "Unable to resolve doctor_id and clinic_id from token or profile."
            )
