...
| CONS-02 | Consultation states: started, in_processing, draft_generated, under_physician_review, finalized | State machine enforced in backend domain layer | State transitions via session and review endpoints | `status` field on consultation record + audit events | Status indicator on consultation detail screen | 006 |
