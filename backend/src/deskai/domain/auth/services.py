...
def compute_entitlements(
    ...
    Args:
        ...
        now: ISO 8601 timestamp for the current time. Defaults to UTC now.

    Returns:
        Computed entitlements with limits, trial status, and capability flags.
    """
    current_time = (
        datetime.fromisoformat(now) if now else datetime.now(tz=UTC)
    )
    ...
    if plan_type == PlanType.FREE_TRIAL:
        account_created = datetime.fromisoformat(created_at)
        trial_end = account_created + timedelta(days=TRIAL_DURATION_DAYS)
    ...
    if monthly_limit == -1:
        ...
        consultations_remaining = max(
            0, monthly_limit - consultations_used_this_month
        )
