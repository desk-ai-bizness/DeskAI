...
class TestPipelineHandler(unittest.TestCase):
    ...
    def test_missing_consultation_id_returns_error(self, mock_build) -> None:
        handler_fn = self._import_handler()
        result = handler_fn({"clinic_id": "clinic-001"}, None)

        self.assertEqual(result["status"], "error")
        self.assertIn("Missing", result["error"])

    @patch(f"{_HANDLER_MODULE}.build_container")
    def test_missing_clinic_id_returns_error(self, mock_build) -> None:
        handler_fn = self._import_handler()
        result = handler_fn({"consultation_id": "cons-001"}, None)
    ...
    @patch(f"{_HANDLER_MODULE}.build_container")
    def test_pipeline_error_returns_error_status(self, mock_build) -> None:
        handler_fn = self._import_handler()
        container = MagicMock()
        ...
        self.assertEqual(result["status"], "error")
        self.assertIn("Consultation not found", result["error"])

    @patch(f"{_HANDLER_MODULE}.build_container")
    def test_unexpected_exception_returns_internal_error(self, mock_build) -> None:
        handler_fn = self._import_handler()
        container = MagicMock()
